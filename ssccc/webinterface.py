...
<html><head><title>BlinkenBlueHouse Webinterface Login</title>
...
'''
        if 'a' in params.keys() and 'rid' in params.keys():
            ...
            if int(params['rid'][0]) == self.server.playlist.request_id:
                if params['a'][0][0] == 'D':
                    self.server.playlist.move_down(int(params['a'][0][1:]))
                elif params['a'][0][0] == 'P':
                    self.server.playlist.inc_repeats(int(params['a'][0][1:]))
                elif params['a'][0][0] == 'M':
                    self.server.playlist.dec_repeats(int(params['a'][0][1:]))
                elif params['a'][0][0] == 'X':
                    self.server.playlist.remove(int(params['a'][0][1:]))
                elif params['a'][0][0] == 'A':
                    self.server.playlist.add(int(params['a'][0][1:]))
            else:
                output += 'Error in playlist change: playlist was changed by '\
                    'another client.<br>\n'
            ...
            if params['s'][0] == 't':
                ...
                for i in range(len(self.server.playlist.list)):
                    ...
                    if anim[0][1] == '':
                        ...
                        comment = anim[0][1]
                    output += '  <tr><td>%s</td><td>%s</td><td>%d</td>\n' % \
                        (anim[0][0], comment, anim[1])
                    for act in (('D', '&darr;'), ('P', '+'),
                                ('M', '-'), ('X', 'X')):
                ...
                for i in range(len(self.server.playlist.available)):
                    ...
                    if anim[1] == '':
                        ...
                        comment = anim[1]
                    output += '''  <tr><td>%s</td><td>%s</td><td><a href="javascript:;" onclick="set_param('s','a');set_param('a','A%d');set_param('rid','%d');submit();">add</a></td></tr>\n''' % \
                        (anim[0], comment, i, rid)
                output += '</table>\n'
                self.server.playlist.lock.release()
